...
public abstract partial class DiagnosticVerifier
{
    ...
    {
        VerifyDiagnostics(new[] { source }, LanguageNames.CSharp, GetCSharpDiagnosticAnalyzer() ?? throw new NotSupportedException(), expected);
    }
    ...
    {
        VerifyDiagnostics(new[] { source }, LanguageNames.VisualBasic, GetBasicDiagnosticAnalyzer() ?? throw new NotSupportedException(), expected);
    }
    ...
    private static void VerifyDiagnosticResults(IEnumerable<Diagnostic> actualResults, DiagnosticAnalyzer analyzer, params DiagnosticResult[] expectedResults)
    {
        ...
        for (int i = 0; i < expectedResults.Length; i++)
        {
            ...
            // Only verify message if we're interested in verifying it
            if (!string.IsNullOrEmpty(expected.Message) && actual.GetMessage() != expected.Message)
            {
                Assert.True(false,
