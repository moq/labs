...
{
    protected override DiagnosticAnalyzer? GetCSharpDiagnosticAnalyzer() => null;
    ...
    protected void VerifyCSharpFix(string[] sources, string fixedSource, int? codeFixIndex = null, bool allowNewCompilerDiagnostics = false)
    {
        VerifyFix(LanguageNames.CSharp,
            GetCSharpDiagnosticAnalyzer() ?? throw new NotSupportedException($"Please override {nameof(GetCSharpDiagnosticAnalyzer)}."),
            GetCSharpCodeFixProvider() ?? throw new NotSupportedException($"Please override {nameof(GetCSharpCodeFixProvider)}."),
            sources, fixedSource, codeFixIndex, allowNewCompilerDiagnostics);
    }
    ...
    protected void VerifyBasicFix(string[] sources, string fixedSource, int? codeFixIndex = null, bool allowNewCompilerDiagnostics = false)
    {
        VerifyFix(LanguageNames.VisualBasic,
            GetBasicDiagnosticAnalyzer() ?? throw new NotSupportedException($"Please override {nameof(GetBasicDiagnosticAnalyzer)}."),
            GetBasicCodeFixProvider() ?? throw new NotSupportedException($"Please override {nameof(GetBasicCodeFixProvider)}."),
            sources, fixedSource, codeFixIndex, allowNewCompilerDiagnostics);
    }
    ...
    void VerifyFix(string language, DiagnosticAnalyzer analyzer, CodeFixProvider codeFixProvider, string[] sources, string fixedSource, int? codeFixIndex, bool allowNewCompilerDiagnostics)
    {
        ...
        for (int i = 0; i < attempts; ++i)
        {
            ...
            document = ApplyFix(document, actions.ElementAt(0)) ?? throw new InvalidOperationException();
